    ...
    try {
      const mongoose  =  await connect()
        ...
        if (!Object.keys(db.models).length) {
            ...
            newsfeed_details =  await newsfeed_model_values.create(req.body)
        }
...
const update_newsfeed_details = asyncWrapper(async(req,res,next) => {
  ...
  try {
    const mongoose  =  await connect()
      ...
      if (!Object.keys(db.models).length) {
          let newsfeed_model_values = db.model('newsfeed',Newsfeed)
          newsfeed_details =  await newsfeed_model_values.findOneAndUpdate({_id:newsfeedId},req.body,{
            new:true,
            runValidatores:true,
          })
          if (!newsfeed_details) {
            return next(createCustomError(`No newsfeed with id : ${newsfeedId}`, 404))
          }
      }
    mongoose.connection.close()
  } catch (error) {
...
const delete_newsfeed_details = asyncWrapper(async(req,res,next) => {
  ...
  try {
    const mongoose  =  await connect()
      ...
      if (!Object.keys(db.models).length) {
          let newsfeed_model_values = db.model('newsfeed',Newsfeed)
          newsfeed_details =  await newsfeed_model_values.findOneAndDelete({_id:newsfeedId})
          if (!newsfeed_details) {
            return next(createCustomError(`No newsfeed with id : ${newsfeedId}`, 404))
          }
      }
